...
function initMap(data) {
  ...
  div.innerHTML = `<p>Best time: ${data.bestTime}</p>`;
  legend.append(div);
  var div = document.createElement("div");
  div.innerHTML = `<p>Last time: ${data.lastTime}</p>`;
  legend.append(div);
  var div = document.createElement("div");
  div.innerHTML = `<p>Total distance: ${data.distance}</p>`;
  legend.append(div);
  var div = document.createElement("div");
  div.innerHTML = `
  <svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 -20 120 240">
    <defs>
  ...
  <p>Total past drivers: ${data.pastTravelCount}</p>`;
  legend.append(div);

  var div = document.createElement("div");
  div.innerHTML = `<div class="flex-c">
  <div class="differece difference-1"></div>
  <div class="differece difference-2"></div>
  <p>DIFFICULTY ${data.difficulty}</p>
  </div>`;
  legend.append(div);

  let total = 100 - parseInt(data.difficulty);
  $(".difference-1").css("width", data.difficulty);
  $(".difference-2").css("width", total + "%");
